...
const SeatBelts = ({ data = {}, onChange }) => {
  ...
  useEffect(() => {
    ...
    seatbeltPanels.forEach((panel) => {
      const existingIssues = data?.[`seatbelt_${panel}_issues`];
      initCondition[panel] = Array.isArray(existingIssues)
        ? existingIssues
        : existingIssues
        ? [existingIssues]
        : [];
      const imgs = Array.isArray(data?.[`seatbelt_${panel}_imageUrls`]) ? data[`seatbelt_${panel}_imageUrls`] : [];
  ...
  const toggleThirdRow = () => {
    ...
    onChange && onChange("seatbelt_third_row_toggle", newVal);
  };
  ...
  const handleConditionChange = (panel, issue) => {
    setCondition((prev) => {
      ...
      onChange && onChange(`seatbelt_${panel}_issues`, updated);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setPhotos((prev) => {
        ...
        onChange && onChange(`seatbelt_${activePanel}_imageUrls`, limitedArr);
    ...
    <div className="bg-[#ffffff0a] backdrop-blur-[16px] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-[0_4px_30px_rgba(0,0,0,0.2)] w-full max-w-4xl mx-auto text-white">
      ...
      <div className="grid grid-cols-1 gap-6 sm:gap-8">
        {seatbeltPanels.map((panel, idx) => {
          const panelEnabled = panel !== "third_row" || thirdRowEnabled;
