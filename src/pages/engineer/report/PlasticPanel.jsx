...
const PlasticPanel = ({ data = {}, onChange }) => {
  ...
  useEffect(() => {
    ...
    plasticPanels.forEach((panel) => {
      const existingIssues = data?.[`plastic_${panel}_issues`];
      initCondition[panel] = Array.isArray(existingIssues)
        ? existingIssues
        : existingIssues
        ? [existingIssues]
        : [];
      const imgs = Array.isArray(data?.[`plastic_${panel}_imageUrls`]) ? data[`plastic_${panel}_imageUrls`] : [];
  ...
  const toggleThirdRow = () => {
    ...
    onChange && onChange("plastic_third_row_toggle", newVal);
  };
  ...
  const handleConditionChange = (panel, issue) => {
    setCondition((prev) => {
      ...
      onChange && onChange(`plastic_${panel}_issues`, updated);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setPhotos((prev) => {
        ...
        onChange && onChange(`plastic_${activePanel}_imageUrls`, limitedArr);
