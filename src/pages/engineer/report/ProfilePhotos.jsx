...
const ProfilePhotos = ({ data, onChange }) => {
  ...
  const [photos, setPhotos] = useState(labels.reduce((acc, label) => ({ ...acc, [label]: null }), {}));
  ...
  useEffect(() => {
    return () => {
      Object.values(streamStates).forEach((stream) => stream && stream.getTracks().forEach((t) => t.stop()));
    };
  }, [streamStates]);

  const toggleDropdown = (label) => setShowDropdown(showDropdown === label ? null : label);

  const handleFileUpload = (e, field) => FileUploaderService.handleFileUpload(e, field, setPhotos, setShowDropdown);
  ...
  const handleCameraClick = (field) =>
    FileUploaderService.handleCameraClick(
      ...
      setStreamStates,
      setIsCameraActive,
      () => FileUploaderService.takePhoto(field, setPhotos, setIsCameraActive, setShowDropdown)
    );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
        {labels.map((label) => (
          <div key={label} className="flex flex-col w-full">
            <label className="text-md text-white font-medium text-left mb-2">{labelNames[label]}</label>
            <div className="mt-2 flex flex-col items-center">
              <div className="relative">
                {photos[label] ? (
                  <img src={photos[label]} alt={labelNames[label]} className="w-24 h-24 object-cover rounded-md cursor-pointer" />
                ) : (
                  <div className="w-24 h-24 bg-gray-700 rounded-md flex items-center justify-center">
                    <button onClick={() => toggleDropdown(label)} className="p-2 rounded-full bg-gray-500 text-white hover:bg-opacity-80">
                ...
                {showDropdown === label && (
                  <div className="absolute top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 rounded-md shadow-lg z-10 w-48">
                    <button onClick={() => handleCameraClick(label)} className="flex items-center px-4 py-3 text-sm text-white hover:bg-gray-700 w-full text-left">
                    ...
                    <label className="flex items-center px-4 py-3 text-sm text-white hover:bg-gray-700 cursor-pointer w-full">
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, label)} />
                    </label>
                  </div>
                )}

                <video ref={(el) => (videoRefs.current[label] = el)} autoPlay className={isCameraActive[label] ? "w-24 h-24 rounded-md" : "hidden"} />
              </div>
            </div>
          </div>
        ))}
