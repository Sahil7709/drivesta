  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (
        ...
        !photoDropdownRef.current.contains(e.target)
      ) {
  ...
  return (
    <div className="w-full">
      ...
      <SelectField
        ...
        onChange={(v) => onFieldChange(tyreKey, "brand", v)}
      />
      ...
      <SelectField
        ...
        onChange={(v) => onFieldChange(tyreKey, "subBrand", v)}
      />
      ...
      <SelectField
        ...
        onChange={(v) => onFieldChange(tyreKey, "variant", v)}
      />
      ...
      <InputField
        ...
        value={size}
        onChange={(v) => onFieldChange(tyreKey, "size", v)}
      />
      ...
      <InputField
        ...
        onChange={(v) => onFieldChange(tyreKey, "manufacturingDate", v)}
      />
      ...
      <InputField
        ...
        value={threadDepth}
        onChange={(v) => onFieldChange(tyreKey, "threadDepth", v)}
      ...
      {issue?.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-4 items-center">
          ...
          {photos.filter((p) => p).length < PHOTO_LIMIT && (
            <div
              className="relative w-24 h-24 flex items-center justify-center rounded-md"
    ...
    tyreKeys.forEach((key) => {
      initial[key] = {
        ...
        photos: Array.isArray(data[`${key}_imageUrls`])
          ? data[`${key}_imageUrls`]
              .slice(0, PHOTO_LIMIT)
              .concat(Array(PHOTO_LIMIT).fill(null))
              .slice(0, PHOTO_LIMIT)
  ...
  const handleFieldChange = (tyreKey, field, value) => {
    setTyreState((prev) => {
      ...
      if (field === "photos") {
        ...
        if (
          tyreKey === "tyre_front_left" && // only trigger if editing front left
          autoCopyFields.includes(field) && // only these fields
          !autoCopied[field] // only once
        ) {
          ["tyre_rear_left", "tyre_rear_right", "tyre_front_right"].forEach(
            (k) => {
              updated[k][field] = value;
              onChange?.(`${k}_${field}`, value); // propagate to parent
            }
          );
              ...
              <div className="mb-4 flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setTyreState((prev) => ({
                      ...prev,
                      [key]: { ...prev[key], toggle: checked },
                    }));

                    // 🔑 Send update to backend schema
                    onChange?.(`${key}_toggle`, checked);
                  }}
