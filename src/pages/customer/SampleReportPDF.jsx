...
//     { label: "4. Front Right View", x: startX + photoBoxWidth + gap, y: mm(150), url: r.front_right_imageUrl },
//     { label: "5. Keys", x: startX + photoBoxWidth + gap, y: mm(120), url: r.keys_imageUrl },
//     { label: "6. Odo", x: startX + photoBoxWidth + gap, y: mm(120), url: r.odo_imageUrl },
//     { label: "7. Engine Number", x: startX + photoBoxWidth + gap, y: mm(120), url: r.engineNumber_imageUrl },
//     { label: "8. VIN Number", x: startX + photoBoxWidth + gap, y: mm(120), url: r.vinNumber_imageUrl },
...
async function addProfilePhotosPage(doc, r) {
  ...
  const pageWidth = A4.w;
  const photoBoxSize = 45;
  const gapX = 8;
  const gapY = 16;
  const cols = 2;
  const rows = 4;
  const gridWidth = cols * photoBoxSize + (cols - 1) * gapX;
  const startX = (pageWidth - gridWidth) / 2;
  const startY = mm(50);
  ...
  for (let i = 0; i < cells.length; i++) {
    const col = i % cols;
    const row = Math.floor(i / cols);
    const x = startX + col * (photoBoxSize + gapX);
    const y = startY + row * (photoBoxSize + gapY);

    labeledPhotoBox(doc, cells[i].label, x, y, photoBoxSize, photoBoxSize);

    if (typeof cells[i].url === "string" && cells[i].url.trim()) {
      // Use urlToDataURL directly on the URL or base64 string from your data
      // without prefixing with ServerUrl.IMAGE_URL
      const data = await urlToDataURL(cells[i].url);
      if (data) {
        const format = data.match(/^data:image\/(\w+);base64,/)?.[1]?.toUpperCase() || "JPEG";
        doc.addImage(
          data,
...
async function addOtherObservationsPage(doc) {
  ...
  const observations = [
    { category: " other_observations", notes: "1. DTC code found U0146 Network Communication Error. \n      2. Scratches found on Rear Left Wheel." },
  ];
